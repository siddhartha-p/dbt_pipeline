{{ config(
    ...
    unique_key = ['client_employee_id', 'punch_apply_date', 'punch_in_datetime']
) }}
...
with remove_non_valid_employee_data as (
    ...
    select
        t.*
    from {{ source('bronze', 'timesheets') }} t
    join {{ source('bronze', 'employees') }} e
...
remove_empty_punch_data as (
    ...
    select *
    from remove_non_valid_employee_data
    where punch_in_datetime <> ''
      and punch_out_datetime <> ''
      and punch_apply_date <> ''
...
replace_null_strings as (
    ...
    select
        ...
        string_to_array(
            regexp_replace(nullif(pay_code, ''), '\s*\|\s*', '|', 'g'),
            '|'
        ) as pay_code,

        string_to_array(
            regexp_replace(nullif(punch_in_comment, '[NULL]'), '\s*\|\s*', '|', 'g'),
            '|'
        ) as punch_in_comment,

        string_to_array(
            regexp_replace(nullif(punch_out_comment, '[NULL]'), '\s*\|\s*', '|', 'g'),
            '|'
        ) as punch_out_comment,
